...
def main():
    ...
    if option_dict['-paired']=="1":
        ...
        for tuple_file in paired:
            ...
            run_RSEM=os.system("rsem-calculate-expression -p %s --paired-end --bowtie2 --estimate-rspd --append-names --output-genome-bam %s %s %s %s" % (option_dict['-cores'], tuple_file[0], tuple_file[1], option_dict['-ref'], tuple_file[0]+"--"+tuple_file[1]+"_rsem_output.txt"))
        ...
        for file in filtered:
            ...
            run_RSEM=os.system("rsem-calculate-expression -p %s --bowtie2 --estimate-rspd --append-names --output-genome-bam %s %s %s" % (option_dict['-cores'], file, option_dict['-ref'], file+"_rsem_output.txt"))
